{*******************************************************************************
  PROJETO: InfoCotidiano - Exemplos Práticos para PDV
  CANAL:   https://www.youtube.com/@InfoCotidiano
  AUTOR:   Daniel de Morais
  DATA:    Maio de 2020

  SISTEMA OPERACIONAL: Multi-plataforma (Windows / Linux)
  COMPILADOR:          Lazarus (Free Pascal)
*******************************************************************************
  DESCRIÇÃO:
    Esta unit faz parte do ecossistema de exemplos do canal InfoCotidiano.
    Contém implementações práticas voltadas para Automação Comercial e Ponto
    de Venda (PDV), incluindo:
      - Pesquisa dinâmica de produtos por descrição/código;
      - Integração com balanças de checkout utilizando componentes ACBrBAL;
      - Decodificação e leitura de etiquetas de balança (padrão de gôndola);

  LICENÇA:
    Código aberto disponibilizado gratuitamente para fins didáticos e de
    estudo. Sinta-se livre para adaptar e utilizar.
*******************************************************************************}
...
type
  ...
  TfrmPrincipal = class(TForm)
    ...
    procedure gridPESQUISAKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
...
uses typinfo,
  ...
  {Para instalar o ACBr usando o Script do Ari, visite meu canal... www.youtube.com/infocotidiano}
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      ShowMessage('Erro ao conectar no banco de dados' + sLineBreak +
        'Mensagem: ' + e.Message);
    end;
  end;
end;
    ...
    begin
      ...
      begin
        try
          ...
          on e: Exception do
          begin
            ShowMessage('L173 - Erro ao ler o peso da balança' +
              sLineBreak + 'Verifique os cabos e se a balança esta ligada' +
...
begin
  ...
  begin
    ...
    case valid of
      0: Memo1.Lines.Text :=
          'TimeOut !' + sLineBreak + 'Coloque o produto sobre a Balança!';
      -1: Memo1.Lines.Text := 'Peso Instavel ! ' + sLineBreak + 'Tente Nova Leitura';
...
var
  LPathDLL, cArquivoDLL, cDataBase: string;
begin
  LPathDLL := ExtractFilePath(ParamStr(0));
  {$ifdef CPU32}
        cArquivoDLL := LPathDLL+ 'SQLiteDLL\x86\sqlite3.dll';
  {$endif}
  {$ifdef CPU64}
        cArquivoDLL :=LPathDLL+ 'SQLiteDLL\x64\sqlite3.dll';
  {$endif}

  cDataBase := LPathDLL + 'banco.db3';

  if FileExists(cDataBase) then
    Conexao.database := cDataBase
  else
    raise Exception.Create('Erro ao lozalizar Banco de dados ' + sLineBreak + cDataBase);
...
begin
  ...
  begin
    ...
    if (nVALOR <> 0) or (nPESO <> 0) then
      // se o peso ou valor for diferente de zero.. entao é nao trata etq como balanca
    begin
      if PesqProd(cCODIGO.Text) then
      begin
        if nVALOR > 0 then
